...
public class ChristmasChestAnimation extends TreasureOpenAnimation
{

	private static final ItemStack HELMET = new ItemStack(Material.ICE);
	...
	public ChristmasChestAnimation(Treasure treasure, TreasureLocation treasureLocation)
	{
	...
	{
		changeFloor(Material.PACKED_ICE, 0, Material.SNOW_BLOCK, 0);
	...
	public void onTick()
	{
		...
		for (ArmorStand stand : _stands)
		{
			...
			UtilParticle.PlayParticleToAll(ParticleType.SNOWBALL_POOF, location.add(0, 1, 0), 0.25F, 0.25F, 0.25F, 0.1F, 1, ViewDist.NORMAL);
		...
		if (_totalTheta >= TOTAL_THETA)
		{
			for (ArmorStand stand : _stands)
			{
				...
				UtilParticle.PlayParticleToAll(ParticleType.SNOW_SHOVEL, location, 0.8F, 0.8F, 0.8F, 0.1F, 20, ViewDist.NORMAL);
				stand.remove();
