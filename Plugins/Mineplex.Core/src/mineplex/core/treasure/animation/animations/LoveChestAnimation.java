...
public class LoveChestAnimation extends TreasureOpenAnimation
{

	private static final ItemStack HELMET = new ItemStack(Material.WOOL, 1, (short) 6);
	...
	public LoveChestAnimation(Treasure treasure, TreasureLocation treasureLocation)
	{
	...
	{
		changeFloor(Material.WOOL, 14, Material.WOOL, 6);
	...
	public void onTick()
	{
		...
		for (ArmorStand stand : _stands)
		{
			...
			UtilParticle.PlayParticleToAll(ParticleType.HEART, location.add(0, 1, 0), 0.25F, 0.25F, 0.25F, 0.1F, 1, ViewDist.NORMAL);
		...
		if (_totalTheta >= TOTAL_THETA)
		{
			for (ArmorStand stand : _stands)
			{
				...
				UtilParticle.PlayParticleToAll(ParticleType.HEART, location, 0.8F, 0.8F, 0.8F, 0.1F, 20, ViewDist.NORMAL);
				stand.remove();
